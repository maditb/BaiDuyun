...
public class  HttpHandler  <T> extends  AsyncTask<Object, Object, Object> implements EntityCallBack{
	...
	private boolean isResume = true; //是否断点续传
	private String charset;
	private long previousDownload = 0;
	private T mDownloadParcel;
	...
	public HttpHandler(AbstractHttpClient client, HttpContext context, AjaxCallBack<T> callback,String charset,T downloadParcel) {
		this.client = client;
		this.context = context;
		this.callback = callback;
		this.charset = charset;
		this.mDownloadParcel = downloadParcel;
	}
	...
	protected void onProgressUpdate(Object... values) {
		...
		case UPDATE_LOADING:
			if(callback!=null){
				//计算速度
				long speed = (Long.valueOf(String.valueOf(values[2])) - previousDownload)/1024/(callback.getRate()/1000);
				((DownloadParcel)mDownloadParcel).speed = speed+"";
				callback.onLoading(Long.valueOf(String.valueOf(values[1])),Long.valueOf(String.valueOf(values[2])),mDownloadParcel);
				previousDownload = Long.valueOf(String.valueOf(values[2]));
			}
			break;
		case UPDATE_FAILURE:
			if(callback!=null)
				callback.onFailure((Throwable)values[1],(Integer)values[2],(String)values[3],mDownloadParcel);
			break;
		case UPDATE_SUCCESS:
			if(callback!=null)
				callback.onSuccess(mDownloadParcel);
			break;
		default:
			break;
		}
